...
|------|--------------------------------------------------------|------------|
| V1   | `frontend-state.yaml -> persistence` (in-memory only)  | Added explicit `carve_outs` entry for the auth token (**rebased to sessionStorage key `staff-engagement:token` to align with ATSE1-41 / PR #37**); bumped YAML to v1.2.0. |
| V7   | `MISSION.md §6` out-of-scope: persistence              | Replaced clause with auth-token-only carve-out referencing the YAML (**rebased to sessionStorage**); bumped to v1.3.0. |
