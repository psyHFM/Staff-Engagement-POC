...
### Frontend state
- `AuthState` (ATSE1-25) — uses `signal()` for token + username, `computed()` for `isAuthenticated`/`currentUser`/`bearerToken`/`currentUserSubject`. Persistence is encapsulated inside the service via the new `AUTH_STORAGE` injection token; components never touch `sessionStorage` directly. The carve-out is recorded in `frontend-state.yaml -> persistence.carve_outs` per the §1.4 ROADMAP §2.7 amendment, with `staff-engagement:token` and `staff-engagement:username` explicitly enumerated (closes the original §2 W1).
