...
- **`InteractionType` storage:** store the enum name (`CHECK_IN`) or the JSON wire name (`check-in`) in the `type` column? Default: store the JSON wire name to match the contract vocabulary; confirm at implementation.